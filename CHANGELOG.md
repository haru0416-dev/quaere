...
- **Install profiles** — `quaere install --profile full|lean|contract|auto` assembles each skill from layer-tagged sections in its single-source `SKILL.md` (`gate` / `method` / `pedagogy`). `lean` (gate+method) matched the full skill's verdicts at 43% of its cost on GPT in a 2-scenario A/B; `contract` (gate only) targets Claude-family agents, whose baseline already passed every restraint scenario unaided. `auto` maps codex→lean, claude→contract. Default stays `full`; untagged skills install as-is under every profile. Pilot conversion: `quaere-evidence`. Design and measurements: `docs/profile-separation.md`.
...
- CI toolchain catch-up: pnpm 9 → 11, pnpm/action-setup v4 → v6, actions/setup-node v4 → v6, Node runtime in CI 22 → 24 (active LTS). `cli/package.json` `engines.node` stays at `>=22` so users on the maintenance LTS line can still install. `sigstore/cosign-installer` stays at `@v3` because the upstream repo does not ship a `v4` major-version alias tag yet.
