...
`quaere install` / `update` / `doctor` の挙動を以下に固定する。Python validator `scripts/validate_skills.py` は CI で別途実行する:
...
- **デフォルトの `--repo` は `haru0416-dev/quaere`**。fork を追っているなら `quaere update --repo your-fork/quaere` で差し替える。
