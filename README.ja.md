...
[なぜ Quaere](#なぜ-quaere) · [スキル](#スキル) · [インストール](#インストール) · [CLI](#インストールの確認と管理) · [quaere.dev](https://quaere.dev/)

これは [README.md](README.md) の日本語版。内容は同じ。英語版が一次資料、こちらは読みやすさのためのミラー。
...
v0.2.0 を 14 シナリオ・90 個の決定的アサーションで通した結果。Codex CLI 0.128.0、baseline と with-skill の両モード、`--scenarios-extra evals/scenarios.ja.json` 付き。独立 2 回:

| モード | レンジ (2 runs) |
| --- | ---: |
| Baseline (スキルなし) | 63.9 – 65.1% |
| **with-skill** | **91.1 – 94.4%** |
| Δ | **+27 – +29 pp** |

v0.1.0 (with-skill 89.8% / Δ +28 pp) との比較で言うと、with-skill の上限が +4.6 pp 伸び、Δ は同じ帯。LLM 出力の確率変動があるので、上の数字は点推定でなくレンジとして読む。

eval ハーネスは `evals/run_skill_evals.py`、14 シナリオは `evals/scenarios.json`、日本語ロケール選言は `evals/scenarios.ja.json`。

ランナーをまたいだ安定性は v0.1.0 測定時に確認済み。同じ `sdk-version-grounding` シナリオを Claude Code 2.1.141 と Codex CLI 0.128.0 で並べたところ、with-skill では両方が 6P/0F で一致した。
...
- 単発の修正ではなく性質と攻撃面から脆弱性を見つけ出す作業なら `quaere-audit`。
...
| 仕様と攻撃面から脆弱性を発見・検証する | `quaere-audit` | 内側で他の四つを必要に応じて呼び分ける |
...
v0.1.0 の CLI は以下の挙動を持つ。

- **`quaere install` は累積的**。同じ `--target` に対して `--skill quaere-semantic` → `--skill quaere-audit` の順で流せば、両方が manifest に蓄積する。manifest は (既存スキル ∪ 今回追加 ∪ 既存重複) の和集合として整合し、エントリーは決定的にソートされる。
...
この契約は `tests/test_validator_parity.py` の Python validator ↔ Rust doctor 同等性チェックで担保している (CI の `parity` ジョブ)。
...
- リスクが高い作業では、見栄えのいいパッチより、根拠の確保と反証プローブを優先する。
...
frontmatter、ディレクトリ・名前の整合、README カバレッジ、行数バジェット、`.agent-state/` の混入をまとめて検査する。push と PR の都度、GitHub Actions が同じ検証を流す。

## スキル評価

`evals/scenarios.json` には 14 個のポータブルシナリオがある。うち 7 つは [`eval-fixtures/`](eval-fixtures/) のワークスペースを使う。ランナーは実行ごとに隔離ディレクトリへ fixture を複製する。並列に流しても干渉しない。

任意のローカルエージェント CLI に対してシナリオを流す例:
...
`--output-dir` は絶対パスにする。ランナーがワークスペース内 `cwd` で `$prompt_file` を読むため、相対だと開けない。

各 run は `eval-results/<timestamp>/` 配下に隔離プロンプト、stdout/stderr、メタデータ、アサーションごとのグレードを書く。`--mode both` は baseline と with-skill の両方を順に走らせる。

ランナーコマンドテンプレートで使える変数:

- `$prompt_file` — 生成プロンプトファイル
- `$output_file` — stdout キャプチャ先
- `$workspace` — 隔離ワークスペース
- `$run_dir` — 結果ディレクトリ
- `$scenario_id`, `$skill`, `$mode` — run メタデータ

アサーション種別:

| 種別 | 役割 |
| --- | --- |
| `contains` / `contains_any` / `not_contains` | リテラル部分文字列の有無 |
| `regex` | 正規表現が一致する箇所が一つでもあるか |
| `ordered_sections` | パターンのリストがその順番で並ぶか |
| `min_section_count` | 同じパターンが最低 N 回現れるか |
...
プロジェクトの公開面は英語に揃えてある。スキル本体、シナリオプロンプト、`evals/scenarios.json` のアサーションパターンも英語のみ。日本語や言い換えの選言が必要な場合は `evals/scenarios.ja.json` に分離し、明示的にオプトインで取り込む:
...
extras は (シナリオ id、アサーション名) で名前マッチしてから合流する。regex や文字列フィールドは `|` 選言で末尾に追加、`ordered_sections.patterns` / `not_in_baseline.patterns` はポジションごとに選言追加、`contains_any.texts` は配列連結。未知の id や名前は早期にエラーで弾くので、古い extras ファイルが選言を黙って取りこぼすことはない。
