...
v0.1.0 を 14 個のシナリオ・88 個の決定的アサーションで通した結果。Codex CLI 0.128.0、baseline (スキルなし) と with-skill の両モードで E2E で走らせている。

| モード | 合格率 | 失敗数 |
| --- | ---: | ---: |
| Baseline (スキルなし) | 61.7% | 31 / 81 |
| **with-skill** | **89.8%** | 9 / 88 |
| Δ | **+28 pp** | |

アサーション種別ごとに分けて見ると傾向が出る。baseline で落ちていた失敗のうち、claim ↔ evidence のペアリング、固定語彙、最小列挙の三種類は with-skill で 100% パスへ反転した。section の順序は 78%。eval ハーネスは `evals/run_skill_evals.py`、シナリオ本体は `evals/scenarios.json` にある。

ランナーをまたいだ安定性も確認している。同じ `sdk-version-grounding` シナリオを Claude Code 2.1.141 と Codex CLI 0.128.0 で並べたところ、with-skill では両方が 6P/0F で一致した。
