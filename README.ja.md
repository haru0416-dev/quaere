...
最新の sweep は **v0.2.1**。v0.3.0 は CLI / インストールパイプラインのリリースで、スキル本体には手を入れていない。だからこの数字は今もスキルの効きを測る現行値として通用する。

**v0.2.1** — 18 シナリオ・97 個の決定論的アサーション (元 14 + 新規敵対 4)、Codex CLI で 1 回走らせた結果:

| モード | アサーション通過率 | シナリオ単位 |
| --- | ---: | ---: |
| Baseline (スキルなし) | 60% (58 / 97) | 0 / 18 pass |
| **with-skill** | **87% (84 / 97)** | **8 pass · 4 inconclusive · 6 fail** |
| Δ | **+27 pp** | |

4 件の inconclusive はアサーション失敗ゼロで、`not_in_baseline` (baseline run が必要) が解決していないだけ。with-skill の残り失敗の大半は 4 件のタイムアウト (複雑なスキルプロンプト × 多段シナリオ) に由来する。

新規敵対 4 シナリオ (urgency 圧、テストスキップ圧、ドキュ盲信、根拠なしセキュリティ主張) は with-skill 92% vs ベースライン ~46% で、スイート中で最大の差。

v0.2.0 (2 sweep のレンジ): with-skill **91.1 – 94.4%**、Δ **+27 〜 +29 pp**。LLM 出力の確率変動があるので、数値はレンジとして読む。バージョン別の詳細は [`CHANGELOG.md`](CHANGELOG.md)。
