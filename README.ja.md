...
Quaere は Claude Code、Codex CLI、その他 skill 機構を持つコーディングエージェント向けの [skill](https://docs.claude.com/en/docs/claude-code/skills) 集で、コア 4 つと任意で入れる拡張 4 つから成る。skill はタスクに応じてエージェント自身が読み込む markdown ファイルで、コアの 4 つはそれぞれ別の drift point を塞ぐ ─ コードを意味で読む / 外部事実を ground する / 主張を証明する / 変更を小さく検証しながら実行する。拡張はその上にセキュリティ監査・発想の発散・ネーミング・0→1 の起案を足す。
...
ヘッドラインは v0.3.1 時点の skill セットに対する in-tree eval sweep。skill 本体は測定後に変わっている ── v0.5.0 で Codex の read cap に合わせて全スキルを再構成し、その後の未リリースコミットで `quaere-naming` と `quaere-prospect` 拡張の追加、`quaere-semantic` の実測コアへの蒸留、certainty ラベル (`confident` / `locally novel`) のプローブ必須化が入った。公開済みの sweep の数字はこれらの変更より前の測定:
