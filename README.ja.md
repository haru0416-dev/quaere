...
このプロジェクトはアーカイブ済みで、今後の更新・保守は行わない。一括導入を勧めるものではなく、参考資料として残している。AI モデルとエージェントの性能向上により、当時の補助手順を追加する必要が薄れた可能性があるためだ。

内容の多くはコードの読解・事実確認・検証などの作業手順で、モデルの通常の動作やエージェント側の指示と重複する場合がある。監査や命名のチェックリスト、テンプレート、命名候補の利用状況を調べるスクリプトも含む。すべてが不要になったと確認したわけではない。

[後期の比較記録](docs/profile-separation.md)では、Sonnet 5 はスキルなしでも報告された6回すべてで正しい判定を出している。token-bucket の課題でも3つの不変条件を自力で発見した。ただし各条件1回の小規模な比較であり、全モデル・全課題に当てはまる結果ではない。下記のアサーション通過率 53% → 91% は古いスキル本文と専用の評価課題で得た数値である。条件は [評価記録](docs/evaluation.md) を参照。

今回の確認はスキル本文と既存の評価記録を読んだ範囲に限る。新しいモデルでの比較実験は行っていない。まずはスキルなしで使い、具体的な不足がある場合に該当する手順だけを選んで試してほしい。以下の説明・測定値・導入手順・ロードマップは開発当時の記録として残す。現在の有効性や互換性、今後のリリースを保証するものではない。
...
- **80 タスク全体で +1.25 pp** (41/80 → 42/80、51.25% → 52.50%)。この測定での差は正味1タスクであり、安定した改善や悪化しないことを証明するものではない。
