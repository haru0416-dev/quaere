...
Quaere は Claude Code、Codex CLI、その他 skill 機構を持つコーディングエージェント向けの [skill](https://docs.claude.com/en/docs/claude-code/skills) 5 つから成る。skill はタスクに応じてエージェント自身が読み込む markdown ファイルで、Quaere の 5 つはそれぞれ別の drift point を塞ぐ ─ コードを意味で読む / 外部事実を ground する / 主張を証明する / 変更を小さく検証しながら実行する / セキュリティ性質を audit してから報告する。
...
ヘッドラインは in-tree eval sweep。skill 本体は v0.3.1 以降変わっていない (v0.3.x / v0.4.x は CLI / インストール / リリース基盤に手を入れた版で、シップ済みスキル自体は据え置き):
