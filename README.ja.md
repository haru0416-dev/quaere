...
別軸として Terminal-Bench (`terminal-bench-core==0.1.1`、80 タスク) も回した: baseline 56.25% → with-skill 38.75% (**Δ −17.5 pp**)。20 件の regression のうち 17 件が `agent_timeout`、`agent_timeout` 自体も 9 → 33 に増えており、品質劣化ではなく Quaere の deliberation pass が時間予算を食い切ってグレーダにカウントされている構造的なもの。詳しくは [`docs/evaluation.md`](docs/evaluation.md)。
